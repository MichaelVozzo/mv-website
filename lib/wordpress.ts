...
import type { Post, Category, Tag, Page, Author, FeaturedMedia, ImageData, ImageReturn } from "./wordpress.d";
...
async function wordpressFetch<T>(url: string, options: FetchOptions = {}): Promise<T> {
...
export async function getAllPosts(filterParams?: { author?: string; tag?: string; category?: string; search?: string }): Promise<Post[]> {
...
export async function getLatestPosts(count: number = 3): Promise<Post[]> {
	...
	return wordpressFetch<Post[]>(url, {
		next: {
			...defaultFetchOptions.next,
			tags: ["wordpress", "posts", "latest-posts"],
		},
	});
}
...
export async function getLatestProjects(count: number = 3): Promise<Post[]> {
	const query: Record<string, any> = {
		_embed: true,
		per_page: count,
		orderby: "date",
	...
	return wordpressFetch<Post[]>(url, {
		next: {
			...defaultFetchOptions.next,
			tags: ["wordpress", "projects", "latest-projects"],
		},
	});
}
...
export async function getImageById(id: number): Promise<ImageReturn | null> {
	...
	try {
		...
		// wordpressFetch returns the parsed ImageData directly or throws an error
		const imageData = await wordpressFetch<ImageData>(url, {
			next: {
				...defaultFetchOptions.next,
				tags: ["wordpress", `media-${id}`],
			},
		...
		// Now imageData is the actual parsed data.
		// Check if we got data and the essential source_url exists.
		if (!imageData || !imageData.source_url) {
			console.warn(`getImageById: Image data or source_url missing for ID ${id}. Data received:`, imageData);
			return null;
		}
		...
		return {
			...
			// although media_details should usually exist for images.
			width: imageData.media_details?.width ?? undefined, // Return undefined if not present
			height: imageData.media_details?.height ?? undefined,
			alt: imageData.alt_text ?? "", // Return empty string if alt_text is missing/null
		};
	} catch (error) {
		// Log the specific error from wordpressFetch or other issues
		if (error instanceof WordPressAPIError) {
			// Handle specific WordPress API errors (like 404 Not Found)
			if (error.status === 404) {
				// It's common for an ID to not exist, maybe don't log as a full error
				console.warn(`getImageById: Media not found (404) for ID ${id}.`);
			} else {
				console.error(`getImageById: WordPress API Error fetching image ID ${id} from ${error.endpoint}: ${error.status} ${error.message}`);
			}
		} else if (error instanceof Error) {
			// Handle generic errors
			console.error(`getImageById: Generic error fetching image ID ${id}:`, error.message, error.stack);
		} else {
			// Handle unexpected error types
			console.error(`getImageById: Unknown error fetching image ID ${id}:`, error);
		}
