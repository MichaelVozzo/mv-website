...
export async function getImageById(id: number): Promise<ImageReturn | null> {
  ...
  try {
    ...
    if (!imageData || !imageData.source_url) {
      console.warn(
        `getImageById: Image data or source_url missing for ID ${id}. Data received:`,
        imageData
      );
      return null;
    }
    ...
    if (error instanceof WordPressAPIError) {
      ...
      if (error.status === 404) {
        ...
        console.error(
          `getImageById: WordPress API Error fetching image ID ${id} from ${error.endpoint}: ${error.status} ${error.message}`
        );
      }
    } else if (error instanceof Error) {
      // Handle generic errors
      console.error(
        `getImageById: Generic error fetching image ID ${id}:`,
        error.message,
        error.stack
      );
    } else {
      ...
      console.error(
        `getImageById: Unknown error fetching image ID ${id}:`,
        error
      );
    }
